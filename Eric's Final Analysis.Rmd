...
# Gameplan:
  ...
  - Try multiple models: multiple linear, decision tree, random forest, boosted tree, mars, svm
...
boost_tree_model <- boost_tree(learn_rate = .301, loss_reduction = 140) %>% 
  set_engine("xgboost") %>% 
  set_mode("regression")
...
## Stack Ensamble: Linear Regression (simple_recipe), Boosted Tree(simple_rec), Random Forest(simple_rec)

### MAIN FOCUS MODELS:
# Linear Model using log_norm_org_recipe
# Simple Recipe Models: 
# Decision Tree
# Linear Model using simple
# Rand Forest Model with tuned parameters specified to mtry = 23
# Boosted Tree Models with tuned parameters from tuning grid: learning rate, loss reduction -- set to .28, and 140
# Stack Model: Linear (using log_norm_org_rec), Random Forest and Boosted Tree both using simple_recipe

model_list <- list(linear_model, r_forest_model, boost_tree_model)
```

Possible models: linear, decision tree, random forest, boosted tree and stack
...
### Another recipe org_rec uses raw data and log_norm_org recipe uses raw data that's log normalized -- data is filtered in org_rec.
...
wf_set <- wf_set %>%
anti_join(tibble(wflow_id = c("filter_rec_linear_reg_2", "filter_rec_rand_forest", "org_rec_rand_forest", "log_norm_org_rec_rand_forest")),
by = "wflow_id")
...
```{r}
## Stack Ensamble: Linear Regression (org_recipe), Boosted Tree(simple_rec), Random Forest(simple_rec) -- 2nd Best Performer
set.seed(10)
...
write_csv(test_preds_stack, "stackpreds5.csv")
```


```{r}
## Stack Ensamble: Linear Regression (log_norm_org_recipe), Boosted Tree(simple_rec), Random Forest(simple_rec) -- Best Performer
set.seed(10)
ctrl_grid <- control_stack_grid()
ctrl_res <- control_stack_resamples()
...
write_csv(test_preds_stack, "teststackpreds2.csv")
```
