...
rec_knn <- recipe(percent_dem ~ ., data = train) %>% step_rm(id) %>% step_corr(all_numeric_predictors(), threshold = tune()) %>% step_mutate_at(all_numeric_predictors(), fn = function(x){x + 1}) %>% step_impute_knn(all_numeric_predictors()) 
...
train_split_strata <- vfold_cv(train, v = 5, strata = percent_dem)
...
boost_tree_model <- boost_tree(min_n = tune(), learn_rate = tune()) %>% set_engine("xgboost") %>% set_mode("regression")
...
recipe_list <- list(log_rec = log_rec, norm_rec = norm_rec, log_norm_rec = log_norm_rec, norm_log_rec = norm_log_rec, box_rec = box_rec)
...
wf_metrics <- metric_set(rmse, rsq, mape)
...
# TEST CODE
# test <- recipe(percent_dem ~ ., data = train) %>% step_rm(id) %>% step_corr(all_numeric_predictors(), threshold = tune()) %>% step_mutate_at(all_numeric_predictors(), fn = function(x){x + 1}) %>% step_impute_knn(all_numeric_predictors()) %>% step_log(all_numeric_predictors(), base = 10)
