---
title: "DRAFT -- Stats 101C -- Regression Project: Predicting the Percentage of Voters"
author: "Eric Gallardo, Avanthika Panchapakesan, Mayerli Cordero-Cortes, Shayan Saadat, Luke Villanueva"
date: "`r Sys.Date()`"
output: pdf_document
---
...
## II. Recipe two: log_norm_org_rec
```{r, eval=F}
...
# 5. Results and Discussion of Final Model


# 6. Appendix: Final Annotated Script
```{r, eval = FALSE}
...
# This recipe takes the log and normalizes all the predictors in original recipe.
# This recipe will be used in our stack for the linear model regression/workflow. 
log_norm_org_rec <- org_rec %>% 
  step_log(all_numeric_predictors(), base = 10) %>% 
  step_normalize(all_numeric_predictors())

## Simple Recipe -- This recipe uses a population percentage based transformation 
# for predictors revolving around age, ethnicity and education.
# Like in the original recipe, missing values are imputed using knn,
# numeric predictors dealing with income and gdp are shifted up by 1 and have the log taken of
# to transform the values since these predictors are heavily right-skewed. 
# Raw data predictors are removed, some transformed predictors are removed as indicated
# by being commented out, and some gdp and education predictors are removed at the end.
...
##  Boosted Tree -- Parameters of learn_rate and loss_reduction were determined by optimizing
# a tuning grid.
# Learn_rate was significant in finding a Boosted Tree model with low rmse,
# but loss_reduction was less important. This model performs cross validation as part of its stack.
...
head(test_preds_stack, n=5)
```

# 7. Appendix: Team Member Contributions
